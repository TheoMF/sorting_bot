...
struct Action
{
  ...
  {
    return std::visit([](auto &v) -> std::type_index
...
public:
                  ...
                  joint_trajectory_publisher::Params &params)
  {
    ...
    in_grasp_translation_grasp_to_pre_grasp_ = Eigen::Map<Eigen::Vector3d>(params.in_grasp_translation_grasp_to_pre_grasp.data(), params.in_grasp_translation_grasp_to_pre_grasp.size());
    in_base_translation_gripper_to_pre_placing_ = Eigen::Map<Eigen::Vector3d>(params.in_base_translation_gripper_to_pre_placing.data(), params.in_base_translation_gripper_to_pre_placing.size());
    state_update_period_ = 1.0 / params.rate;
    in_world_rot_top_grasp_quat_ = Eigen::Quaterniond(params.in_world_rot_top_grasp_quat[0], params.in_world_rot_top_grasp_quat[1], params.in_world_rot_top_grasp_quat[2], params.in_world_rot_top_grasp_quat[3]);
    in_world_rot_front_grasp_quat_ = Eigen::Quaterniond(params.in_world_rot_front_grasp_quat[0], params.in_world_rot_front_grasp_quat[1], params.in_world_rot_front_grasp_quat[2], params.in_world_rot_front_grasp_quat[3]);
    ...
    // Set in_world_M_box.
    Eigen::Quaterniond in_world_rot_box_quat = Eigen::Quaterniond(params.in_world_pose_box[3], params.in_world_pose_box[4], params.in_world_pose_box[5], params.in_world_pose_box[6]);
    Eigen::Vector3d in_world_translation_world_to_box = Eigen::Vector3d(params.in_world_pose_box[0], params.in_world_pose_box[1], params.in_world_pose_box[2]);
    ...
    for (std::string &base_waypoint_name : params.base_waypoint_names)
    {
      ...
      Eigen::Vector3d base_waypoint = Eigen::Map<Eigen::Vector3d>(
          base_waypoint_vec.data(), base_waypoint_vec.size());
    ...
    {
      std::vector<double> in_box_frame_translation_box_frame_to_box_center_vec = params.box_frames_map.at(box_frame).in_box_frame_translation_box_frame_to_box_center;
      Eigen::Vector3d in_box_frame_translation_box_frame_to_box_center = Eigen::Map<Eigen::Vector3d>(
          in_box_frame_translation_box_frame_to_box_center_vec.data(), in_box_frame_translation_box_frame_to_box_center_vec.size());
    ...
    std::vector<std::vector<double>>
        qs_searching_objects = {params.q_searching_object, params.q_searching_object, params.q_searching_object};
  ...
  void initialize_in_box_M_compartment_map(const double &box_width, const double &box_length, const double &object_height_while_placing_in_box)
  {
  ...
  void do_search_box_action()
  {
    ...
    if (optional_det.has_value())
    {
      ...
      in_base_M_box_.translation() = in_base_M_box_.translation() +
  ...
  void compute_trajectory()
  {
    ...
    else if (state_ == GOING_ABOVE_OBJECT_POSE)
    {
      ...
      double current_gripper_yaw_angle = std::atan2(current_in_base_M_gripper.translation()[1], current_in_base_M_gripper.translation()[0]);
      ...
      Eigen::AngleAxisd z_angle_axis_rot = Eigen::AngleAxisd(des_gripper_yaw_angle - current_gripper_yaw_angle, Eigen::Vector3d::UnitZ());
      pinocchio::SE3 des_in_base_M_gripper = pinocchio::SE3(z_angle_axis_rot.toRotationMatrix() * current_in_base_M_gripper.rotation(), z_angle_axis_rot.toRotationMatrix() * current_in_base_M_gripper.translation());
    ...
    else if (state_ == GRASPING)
    {
      ...
      pinocchio::SE3 in_base_M_grasp = get_in_base_M_grasp(current_object_to_sort_.in_base_M_frame.value(), current_object_to_sort_.frame);
      ...
      if (grasp_from_top)
      {
        ...
        in_base_translation_grasp_to_pre_grasp = in_base_M_grasp.rotation() * x_rot_pi_2_angle_axis.toRotationMatrix() * in_grasp_translation_grasp_to_pre_grasp_;
      }
      else
      ...
      pinocchio::SE3 in_base_M_pre_grasp = pinocchio::SE3(in_base_M_grasp.rotation(), in_base_M_grasp.translation() + in_base_translation_grasp_to_pre_grasp);
    ...
    else if (state_ == PLACING)
    {
      ...
      pinocchio::SE3 in_base_M_pre_placing = pinocchio::SE3(in_base_M_ee.rotation(), in_base_M_ee.translation() + in_base_translation_gripper_to_pre_placing_);
...
private:
  ...
  std::vector<StateMachine> state_order_ = {SEARCHING_OBJECTS, GOING_ABOVE_OBJECT_POSE, GRASPING, SEARCHING_BOX, PLACING};
  ...
  double box_dist_while_placing_, search_box_y_dist_threshold_, wait_duration_before_vision_action_, time_ = 0., state_update_period_, closed_gripper_position_, open_gripper_position_, wait_duration_after_gripper_moved_;
  ...
  pinocchio::SE3 in_world_M_box_, in_base_M_box_, new_transform = pinocchio::SE3::Identity(), first_transform = pinocchio::SE3::Identity(), sec_transform = pinocchio::SE3::Identity();
