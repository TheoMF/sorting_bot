...
public:
  JointTrajectoryPublisher() : Node("joint_trajectory_publisher") {
    ...
    odom_subscriber_ = this->create_subscription<Odometry>(
        ...
        std::bind(&JointTrajectoryPublisher::odom_callback, this, std::placeholders::_1));
    ...
    joint_traj_pub_timer_ =
        this->create_wall_timer(std::chrono::milliseconds((int)(1000.0 / params_.rate)),
...
private:
  ...
  void odom_callback(const Odometry &msg) { base_pose_ = pose_msg_to_base_pose(msg.pose.pose); }
